...
from app.pipeline.data_loading import load_and_preprocess_image
from app.pipeline.recognize_text import detect_text, get_img_no_text, link_text_to_elements
from app.pipeline.recognize_node import detect_shapes, get_nodes_mask
from app.pipeline.commons import fill_contours, remove_contours, dilate_contour, here
from app.pipeline.recognize_arc import (get_hough_lines, HoughBundler, assign_proximity_nodes,
                          get_entry_points_from_lines, find_line_paths, 
                          assign_arrowheads, get_arcs)
from app.pipeline.recognize_arrow import detect_arrowheads
...
def recognize_graph(image_path: str, config_path: str) -> Dict[str, Any]:
    ...
    for transition in transitions:
        width_multiplier = proximity_thres_trans_width if transition.width < transition.height else proximity_thres_trans_height
        height_multiplier = proximity_thres_trans_height if transition.width < transition.height else proximity_thres_trans_width
        expanded_height = transition.height * height_multiplier
        expanded_width = transition.width * width_multiplier
        expanded_bbox_contour = cv2.boxPoints(((float(transition.center.x), float(transition.center.y)),
                                                                    (expanded_width, expanded_height), transition.angle))
...
if __name__ == "__main__":
    from config.path_config import ensure_directories_exist, get_visualization_path
    
    # config_path = DEFAULT_CONFIG_PATH
    config_path = here("../../config/algorithm_config/demo_1_config.yaml")
    image_path = here("../../data/demos/demo_1.png")  # This is just an example path
