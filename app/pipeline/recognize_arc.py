...
import numpy as np
import cv2
from app.domain.models.models import Line, Point, Place, Transition, Arc
        ...
        for line_point in [line.point1, line.point2]:
            for node_center_copy in all_copied_node_centers:
                ...
                if isinstance(node_copy, Place):
                    ...
                    width_multiplier = proximity_thres_trans_width if node_copy.width < node_copy.height else proximity_thres_trans_height
                    height_multiplier = proximity_thres_trans_height if node_copy.width < node_copy.height else proximity_thres_trans_width
                    expanded_height = node_copy.height * height_multiplier
                    expanded_width = node_copy.width * width_multiplier
                    ...
                    expanded_bbox_contour = cv2.boxPoints(((float(node_center_copy.x), float(node_center_copy.y)),
                                                            (expanded_width, expanded_height), node_copy.angle))
