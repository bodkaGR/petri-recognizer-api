...
class RenderFacade:
    ...
    async def render_from_upload(self, file: UploadFile):
        ...
        file_format = FileFormat.from_file_path(file.filename)

        # 2. Save received file to temporary location
        file_path = FileHandler.save_upload_tmp(file, ".".join(file_format.value))

        try:
            # 3. Render uploaded file to image
            output_path = self.renderer_service.render_to_image(file_path, file_format)
            return output_path
        finally:
